...
日文 Mod v2.4 的文件解压至游戏目录后，加载流程在进入 Chainloader 初始化前即中断。最终验证可行的配置如下：
...
修复后，Chainloader 成功加载全部 5 个插件并注入 72,977 行日文文本。原本在此环境下失效的 DiceMaster 模组，通过相同的 Doorstop / corlibs / BepInEx Wine 修复路径（即应用程序级 DLL override、匹配的完整 Unity corlibs 与修复版 BepInEx 核心）也同样得到了修复并正常加载。
...
安装前需对 ZIP 压缩包进行完整性与安全性检查：排查绝对路径与 `..` 路径穿越风险、确认原生 DLL 为 x86 架构，并评估与现有文件的覆盖冲突。首次下载时因混用不同断点续传工具导致 Central Directory 损坏，随后重新通过单一工具完整下载并校验通过。

将所有将被覆盖的现有文件备份至带时间戳的目录。解压合并后重新校验文件哈希值，确认原有的 DiceMaster DLL 及配置文件哈希值保持完整。
...
此设置使进程成功加载本地的 `winhttp.dll` 与 Preloader。但此时加载流程依然停留在 Chainloader 初始化前，表明下一个阻塞点位于托管入口阶段。
...
通过合法渠道获取与游戏 Unity 版本完全一致的完整 corlibs，放入 `BepInEx/unstripped_corlib/` 目录，并配置 `doorstop_config.ini`：
...
在详细日志中确认 Preloader 成功调用并输出 `Done`。corlibs 必须严格匹配 Unity 引擎版本，并在遵守 Unity 软件许可协议的前提下获取与使用。
